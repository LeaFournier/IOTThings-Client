...
function MyHome() {
  ...
  const url =
    "http://35.176.229.91:8080/api/homes/" + localStorage.getItem("userHomeId");
  ...
  //Function that displays certain data from the database
  useEffect(() => {
    const asyncFn = async () => {
  ...
  //Function that allows to modify an item in the database via the API
  function updateHome(e) {
    e.preventDefault();
    Axios.put(url, item, { headers }).then((res) => {
      console.log(res);
      if (res === "Error") {
        window.location.replace(`http://localhost:3000`);
      }
    ...
    <div>
      <MainContext.Provider value={{ navbarOpen, setNavbarOpen }}>
        ...
        <div
          ...
          style={{
            width: !navbarOpen ? "calc(100% - 78px);" : "calc(100% - 240px);",
        ...
        >
          ...
          <div id="col3">
            ...
            ></div>
            <button className="HomeSubmit" onClick={updateHome}>
              Submit
            </button>
          </div>
        </div>
