...
export default (props) => {
  ...
  //Toogle button to activate the sidebar
  const menuActive = () => {
  ...
  //Function that allows to disconnect by deleting the localStorage of the session and redirecting to the login page 
  const handleLogOut = () => {
    Axios.get(url, {
      headers: {
  ...
  //Redirects to the login page if the token is not good 
  if (!token) {
    window.location.replace(`http://localhost:3000`);
  }
        ...
        <div className={isActive ? "sidebar active" : "sidebar"} id="sidebar">
          <div className="logo_content">
            <div className="logo">
              ...
              <div className="logo_name" style={{ marginLeft: "2px" }}>
                IOTThings
              </div>
            </div>
            <div className="menu">
